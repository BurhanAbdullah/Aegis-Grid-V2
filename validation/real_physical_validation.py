...
# When this file is executed directly (``python validation/real_physical_validation.py``),
# Python places ``validation/`` rather than the repository root on sys.path.  Add the
# repository root explicitly so the authoritative ``core`` package is imported from this
# checkout instead of relying on the caller's working directory.
...
def check(name):
    ...
    p_loss = float(np.sum(br[:, 13]+br[:, 15]))
    q_loss = float(np.sum(br[:, 14]+br[:, 16]))
