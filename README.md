...
The branch `scientific-fix-01-canonical-benchmarks` is the active scientific-repair branch. Existing `results/independent_validation_run/` and `results/tsg_run_002/` outputs are retained as historical evidence only. They were generated before the current benchmark-provenance and topology-outage corrections and therefore must not be presented as the regenerated results of the repaired implementation.

A result is considered authoritative only when it is regenerated by the current code and passes the GitHub Actions scientific-validation workflow.
