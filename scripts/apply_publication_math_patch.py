...
REPLACEMENTS = [
    (
        "A sequential innovation accumulator $\\Theta(k)=0.9\\Theta(k-1)+a_{\\mathrm{nis}}(k)$ for persistent anomalies.",
        "A sequential threat accumulator $\\Theta_k=\\alpha\\Theta_{k-1}+S_{\\mathrm{comp},k}$ for persistent composite anomalies.",
    ),
    (
    ...
    (
        ...
        "The sequence threshold is calibrated from baseline data only, avoiding test-set threshold leakage. In the implementation, $\\alpha=0.90$ and the update uses the continuous composite threat score $S_{\\mathrm{comp},k}$ rather than a binary detector flag.",
    ),
    (
    ...
    (
        ...
        "../results/authoritative_validation_20260815/paper_figures/",
    ),
    (
    ...
    ("85.85\\% recall", "the earlier high-recall aggregate"),
    ("recall of $85.85\\%$", "the earlier high-recall aggregate"),
]
...
def main() -> None:
    ...
    for old, new in REPLACEMENTS:
        if old in text:
            ...
            # Idempotent: an already-corrected manuscript is acceptable.
