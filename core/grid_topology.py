...
def get_ieee_case_data(case_name: str) -> Dict[str, Any]:
    ...
    return {"case_name": case_name, "source": "PYPOWER/MATPOWER standard case definition",
            "baseMVA": float(mpc["baseMVA"]), "bus": bus, "branch": branch,
...
def build_ybus(case_data: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    base_mva = float(case_data["baseMVA"]); n = int(case_data["num_buses"])
    ...
    for row in branch:
        ...
        y = 1.0 / z; b_shunt = complex(0.0, row[4])
        ratio = float(row[8]) or 1.0; tap = ratio * np.exp(1j*np.deg2rad(float(row[9])))
        yff = (y+b_shunt/2)/(tap*np.conj(tap)); yft = -y/np.conj(tap)
        ytf = -y/tap; ytt = y+b_shunt/2
...
def compute_jacobian_H(x: np.ndarray, G: np.ndarray, B: np.ndarray) -> np.ndarray:
    """Exact analytical Jacobian H=dh/dx for the AC measurement equation."""
    n = G.shape[0]; x = np.asarray(x, dtype=float)
    if x.size != 2*n-1:
        raise ValueError(f"State dimension mismatch: expected {2*n-1}, got {x.size}")
    theta = np.zeros(n); theta[1:] = x[:n-1]; v = x[n-1:]
    d = theta[:,None]-theta[None,:]; c,s = np.cos(d),np.sin(d); vv=v[:,None]*v[None,:]

    # Angle derivatives (j != i), followed by the diagonal identity from
    # d(theta_i-theta_j)/d(theta_i) = -d(...)/d(theta_j).
    hp_t = vv*(G*s-B*c); hq_t = -vv*(G*c+B*s)
    np.fill_diagonal(hp_t,0.0); np.fill_diagonal(hq_t,0.0)
    np.fill_diagonal(hp_t,-np.sum(hp_t,axis=1)); np.fill_diagonal(hq_t,-np.sum(hq_t,axis=1))

    # Voltage derivatives for j != i use V_i. The diagonal derivative is
    # sum_j V_j * term_ij plus the derivative of the leading V_i factor.
    p_term = G*c+B*s; q_term = G*s-B*c
    hp_v = v[:,None]*p_term; hq_v = v[:,None]*q_term
    np.fill_diagonal(hp_v,0.0); np.fill_diagonal(hq_v,0.0)
    p_diag = p_term @ v + v*np.diag(G)
    q_diag = q_term @ v - v*np.diag(B)
    np.fill_diagonal(hp_v,p_diag); np.fill_diagonal(hq_v,q_diag)

    h_v = np.hstack([np.zeros((n,n-1)),np.eye(n)])
    return np.vstack([h_v,np.hstack([hp_t[:,1:],hp_v]),np.hstack([hq_t[:,1:],hq_v])])
